...
if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_2_5() ) {
	...
	add_action( 'woocommerce_product_after_variable_attributes', 'wc_price_calculator_product_after_variable_attributes', 10, 3 );
}
	...
	?>
		<p class="form-row form-row-first hide_if_variation_virtual">
			<label>
				...
				<?php echo SV_WC_Plugin_Compatibility::wc_help_tip( __( 'Overrides the area calculated from the width/length dimensions for the Measurements Price Calculator.', 'woocommerce-measurement-price-calculator' ) ); ?>
			</label>
		...
		<p class="form-row form-row-last hide_if_variation_virtual">
			<label>
				...
				<?php echo SV_WC_Plugin_Compatibility::wc_help_tip( __( 'Overrides the volume calculated from the width/length/height dimensions for the Measurements Price Calculator.', 'woocommerce-measurement-price-calculator' ) ); ?>
			</label>
	...
	if ( isset( $_POST['variable_sku'] ) ) {
		...
		for ( $i = 0; $i <= $max_loop; $i++ ) {
			...
			if ( empty( $variable_area[ $i ] ) ) {
				delete_post_meta( $variation_id, '_area' );
			} else {
			...
			if ( empty( $variable_volume[ $i ] ) ) {
				delete_post_meta( $variation_id, '_volume' );
			} else {
