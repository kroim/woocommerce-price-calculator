...
class WC_Price_Calculator_Settings {
	...
	public function __construct( $product = null ) {
		...
		if ( $product ) {
			...
			$settings = $product->wc_price_calculator;
		}
		...
		if ( 'dimension' == $calculator_type ) {
			...
			foreach ( $this->settings[ $this->get_calculator_type() ] as $name => $value ) {
				...
				if ( 'pricing' != $name ) {
					...
					$measurement = new WC_Price_Calculator_Measurement( $value['unit'], 1, $name, $value['label'], $value['editable'], $this->get_options( $name ), $value['min'], $value['max'] );
