...
class WC_Price_Calculator_Product {
	...
	public static function calculator_enabled( $product ) {
		...
		// basic checks
		if ( ! $product || $product->is_type( 'grouped' ) || ! $product->wc_price_calculator ) {
			return false;
		}
	...
	public static function get_area_measurement( $product ) {
		...
		if ( ! $measurement ) {
			$unit        = get_option( 'woocommerce_area_unit' );
			$measurement = new WC_Price_Calculator_Measurement( $unit, 0, 'area', __( 'Area', 'woocommerce-measurement-price-calculator' ) );
		}
	...
	public static function get_perimeter_measurement( $product ) {
		...
		if ( $product->length && $product->width ) {
			...
			$unit        = get_option( 'woocommerce_dimension_unit' );
			$measurement = new WC_Price_Calculator_Measurement( $unit, $perimeter, 'length', __( 'Perimeter', 'woocommerce-measurement-price-calculator' ) );
		}

		// if no measurement, just create a default empty one
		if ( ! $measurement ) {
			$unit        = get_option( 'woocommerce_dimension_unit' );
			$measurement = new WC_Price_Calculator_Measurement( $unit, 0, 'length', __( 'Perimeter', 'woocommerce-measurement-price-calculator' ) );
		}
	...
	public static function get_surface_area_measurement( $product ) {
		...
		if ( $product->length && $product->width && $product->height ) {
			...
			$unit        = get_option( 'woocommerce_dimension_unit' );
			$measurement = new WC_Price_Calculator_Measurement( $unit, $surface_area, 'area', __( 'Surface Area', 'woocommerce-measurement-price-calculator' ) );
		}

		// if no measurement, just create a default empty one
		if ( ! $measurement ) {
			$unit        = get_option( 'woocommerce_dimension_unit' );
			$measurement = new WC_Price_Calculator_Measurement( $unit, 0, 'area', __( 'Surface Area', 'woocommerce-measurement-price-calculator' ) );
		}
	...
	public static function get_weight_measurement( $product ) {
		return new WC_Price_Calculator_Measurement( get_option( 'woocommerce_weight_unit' ), $product->get_weight(), 'weight', __( 'Weight', 'woocommerce-measurement-price-calculator' ) );
	}
	...
	public static function calculate_price( $product, $measurement_needed_value, $measurement_needed_value_unit, $round = false ) {
		 ...
		 *
		 * @since 3.10.1
		 * @param bool if true, the returned price will be rounded to two decimal places.
		 * @param WC_Product $product the product.
		 */
		...
		if ( $price > 0 ) {
			...
			if ( $settings->pricing_rules_is_on_sale() && $min_regular_price !== $price ) {
				...
				if ( $min_price !== $max_price ) {
					...
					$from        = wc_price( $min_regular_price ) . ' - ' . wc_price( $max_regular_price ) . ' ' . $pricing_label;
					$to          = __( 'Free!', 'woocommerce-measurement-price-calculator' ) . ' - ' . wc_price( $max_price ) . ' ' . $pricing_label;
					$price_html .= self::get_price_html_from_to( $from, $to, '' ) . $product->get_price_suffix();

				} else {
					$price_html .= self::get_price_html_from_to( $min_regular_price, __( 'Free!', 'woocommerce-measurement-price-calculator' ), $pricing_label );
				}
