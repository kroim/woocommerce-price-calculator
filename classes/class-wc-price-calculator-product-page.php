...
class WC_Price_Calculator_Product_Page {
	...
	/**
	 * Convert a variaton's price to a per unit price
		...
		if ( WC_Price_Calculator_Product::pricing_per_unit_enabled( $_product ) ) {
			...
			if ( $settings->is_quantity_calculator_enabled() ) {
				...
				if ( $product->is_type( 'variable' ) ) {
					...
					// add units
					$price_html .= ' ' . __( $settings->get_pricing_label(), 'woocommerce-measurement-price-calculator' );
	...
	public function available_variation( $variation_data, $product, $variation ) {
		...
		$variation_data['price'] = $variation->get_display_price();
	 ...
	 *
	 * @since 3.10.1
	 * @param int/string/null The max value
	...
	public function enqueue_frontend_scripts() {
		...
		$wc_price_calculator_params = array(
			...
			'minimum_price'                  => $product->wc_measurement_price_calculator_min_price ? $product->get_display_price( $product->wc_measurement_price_calculator_min_price ) : '',
			'measurement_type'               => $settings->get_calculator_type(),
		);

		// information required for either pricing or quantity calculator to function
		$wc_price_calculator_params['product_price'] = $product->is_type( 'variable' ) ? '' : $product->get_display_price();
	...
	public function render_price_calculator() {
		...
		if ( WC_Price_Calculator_Product::pricing_calculator_enabled( $product ) ) {
			...
			if ( $product->is_type( 'simple' ) ) {
				...
				$total_price = wc_price( $quantity_range['min_value'] * $product->get_display_price(), 2 );
