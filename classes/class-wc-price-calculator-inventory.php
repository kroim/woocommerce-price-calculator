...
class WC_Price_Calculator_Inventory {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_backordered_item_meta_name',      array( $this, 'get_backordered_item_meta_name' ), 10, 2 );
				...
				if ( $product->get_total_stock() > 0 ) {
					...
					switch ( $format_option ) {
						...
						case 'low_amount' :
							...
							$format = ( $product->get_total_stock() <= $low_amount ) ? __( 'Only %s %s left in stock', 'woocommerce-measurement-price-calculator' ) : __( 'In stock', 'woocommerce-measurement-price-calculator' );
						break;
						default :
					...
					$availability = sprintf( $format, $product->get_total_stock(), $settings->get_pricing_unit() );
	 ...
	 */
	public function get_backordered_item_meta_name( $backordered, $cart_item_key ) {
